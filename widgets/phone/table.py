...
class ListPhones(tk.Frame):
    def __init__(self, parent, table_phone: PhoneTable):
        ...
        self.treePro = TreeviewPro(self, self.table_phone )
    ...
    def item_selected(self, event):
        ...
        if len(res_str) != 0:
            self.btn_dell['command'] = lambda: self.delete_phone(res_str[0])
            self.btn_copy['command'] = lambda: self.copy_phone_data(res_str)
            self.btn_edit['command'] = lambda: self.form_control.set_edit_data_to_form({'select_row': res_str}) # Первод в режым редактирования
            
    def delete_phone(self, id_phone):
        ...
        if res:
            self.table_phone.dell_phone_by_id(int(id_phone))
            self.treePro.refresh_treeview()
            self.treePro.clear_selection() # Снять выделения с несуществующего елемента в таблице
            self.update()
            Toast(self, "✅ Готово", f'id:{id_phone} успешно удален')

    def copy_phone_data(self, phone):
